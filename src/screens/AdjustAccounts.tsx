          ...
          <div className="w-full sm:w-full sm:max-w-sm bg-zinc-900 rounded-t-3xl sm:rounded-2xl border border-white/10 p-6 space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3">
              {displayAccounts.map((account) => {
                const { icon: Icon, iconColor, container } = getAccountStyles(account.type);
                const currentValue = state.accountsBalances?.[account.id] || 0;
                ...
                return (
                  <div
                    key={account.id}
                    className={`rounded-xl border p-4 transition-colors ${changed  ? 'bg-emerald-500/10 border-emerald-500/30' : `bg-zinc-900/60 border-white/5 ${container}`}`}>
